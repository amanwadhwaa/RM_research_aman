...
This folder contains a lightweight two-layer Retrieval-Augmented Generation (RAG)
demo used for experiments.

Layers
- Layer 1 — Semantic Retrieval: `embeddings.py`, `vector_store.py`, `retrieval.py`
  - Embeddings: deterministic, dependency-free vectors (`text_to_embedding`).
  - Vector storage: `InMemoryVectorStore` with cosine similarity search.
  - Retriever: high-level API that exposes `retrieve()` and `keyword_search()`.

- Layer 2 — RAG (retrieval + generation): integrated in `experiments.py`.
  - Uses retrieved context to build a prompt that instructs the LLM to answer
    only from provided context.
  - Uses existing `llm_client.call_gemini` for generation. Set `GEMINI_API_KEY` to enable.

Experiments (in `experiments.py`)
- 5: Semantic retrieval vs keyword matching
- 6: Grounded generation using retrieved context
- 7: Failure case demonstrating irrelevant retrieval / context overflow
- 8: How retrieval quality affects generation

How to run
1. (Optional) Set `GEMINI_API_KEY` in your environment to enable model calls.

On Windows PowerShell:

```powershell
$env:GEMINI_API_KEY = "your_api_key_here"
python -m llm_module.experiments
```

Notes & Limitations
- Embeddings here are deterministic hashes for offline demos. Replace
  `embeddings.text_to_embedding` with a real embedding API for production.
- This demo intentionally separates embedding generation, vector storage,
  similarity computation, and query handling (see `retrieval.py`).
- Do not use LangChain/agents for this homework; this repo avoids those.

If you want, I can switch embeddings to use an official API (OpenAI / Gemini)
or add FAISS-backed storage. Which would you prefer?
# RoboLLM
...
FAISS (Optional)
-----------------
To enable FAISS for faster, scalable vector search:
...
- On Windows PowerShell, enable FAISS use for this demo by setting:

```powershell
$env:USE_FAISS = "1"
```

When enabled, the retriever will use a FAISS-backed store (`faiss_vector_store.py`).
If FAISS isn't available the system will gracefully fall back to the in-memory store.
