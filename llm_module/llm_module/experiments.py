...
def _build_rag_prompt(query: str, retrieved: list) -> str:
    ctx_parts = []
    for i, r in enumerate(retrieved, start=1):
        ctx_parts.append(f"[{i}] (score={r.get('score'):.3f}) {r.get('text')}")
    context_block = "\n".join(ctx_parts)
    prompt = (
        "You are an assistant that MUST answer questions only using the provided "
        "context below. If the answer is not contained in the context, respond: 'I don't know based on the provided context.'\n\n"
        "Context:\n" + context_block + "\n\n"
        "Question: " + query + "\n\n"
        "Answer concisely and cite which context item you used (e.g., [1])."
    )
    return prompt


def rag_generate(query: str, retriever: Retriever, api_key: str, top_k: int = 3) -> dict:
    """Run a simple RAG flow: retrieve, build prompt, call LLM, return response and retrieved items."""
    retrieved = retriever.retrieve(query, top_k=top_k)
    prompt = _build_rag_prompt(query, retrieved)
    response_text = call_gemini(api_key, prompt)
    return {'query': query, 'retrieved': retrieved, 'prompt': prompt, 'response': response_text}


def experiment_5_semantic_vs_keyword(retriever: Retriever):
    print_section("EXPERIMENT 5: Semantic Retrieval vs Keyword Matching")
    query = "Which city is known for the Eiffel Tower?"
    print_subsection("Query")
    print(query)

    print_subsection("Keyword search results (baseline)")
    kw = retriever.keyword_search(query, top_k=3)
    for r in kw:
        print(f"- score={r['score']}: {r['text']}")

    print_subsection("Semantic search results (retriever)")
    sem = retriever.retrieve(query, top_k=3)
    for r in sem:
        print(f"- score={r['score']:.3f}: {r['text']}")

    print("\nObservation: Semantic retrieval should return the Paris/Eiffel Tower sentence even if keywords differ.")


def experiment_6_grounded_generation(retriever: Retriever):
    print_section("EXPERIMENT 6: Grounded Generation Using Retrieved Context")
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        logger.warning("GEMINI_API_KEY not set. Skipping generation steps for RAG demos.")
        return

    query = "Where is the Eiffel Tower located?"
    result = rag_generate(query, retriever, api_key, top_k=3)
    print_subsection("Retrieved context")
    for r in result['retrieved']:
        print(f"- score={r['score']:.3f}: {r['text']}")

    print_subsection("Prompt sent to model")
    print(result['prompt'])

    print_subsection("Model response (grounded)")
    print(result['response'])


def experiment_7_failure_case(retriever: Retriever):
    print_section("EXPERIMENT 7: Failure Case - Irrelevant Retrieval / Context Overflow")
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        logger.warning("GEMINI_API_KEY not set. Skipping generation steps for RAG demos.")
        return

    # Query that is ambiguous or not covered in corpus
    query = "Who discovered penicillin and when?"
    print_subsection("Query")
    print(query)

    # Retrieve top 5 which will be irrelevant for this query
    result = rag_generate(query, retriever, api_key, top_k=5)
    print_subsection("Retrieved (likely irrelevant)")
    for r in result['retrieved']:
        print(f"- score={r['score']:.3f}: {r['text']}")

    print_subsection("Model response (should decline to answer or hallucinate based on prompt)")
    print(result['response'])


def experiment_8_retrieval_quality_effect(retriever: Retriever):
    print_section("EXPERIMENT 8: Retrieval Quality Impact on Generation")
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        logger.warning("GEMINI_API_KEY not set. Skipping generation steps for RAG demos.")
        return

    query = "Tell me about the longest river in Africa."
    print_subsection("High-quality retrieval (top 1)")
    good = rag_generate(query, retriever, api_key, top_k=1)
    print(good['response'])

    print_subsection("Lower-quality retrieval (top 3 including weaker matches)")
    bad = rag_generate(query, retriever, api_key, top_k=3)
    print(bad['response'])

    print("\nObservation: Differences show how additional, lower-quality contexts can dilute or confuse generation.")
...
✓ Requirement 5: Non-Determinism Awareness
  ...
  - Handled by strict format + validation + retry
    ...
    print_section("RAG EXPERIMENTS: Semantic Retrieval + Grounded Generation")
    # set up a small demo corpus
    retriever = Retriever()
    demo_texts = [
        ...
        "Python is a programming language that emphasizes readability and has significant whitespace.",
        "The capital of France is Paris, famous for the Eiffel Tower.",
        "Tesla is an electric car company founded by Elon Musk and others."
    ]
    retriever.add_texts(demo_texts)

    experiment_5_semantic_vs_keyword(retriever)
    experiment_6_grounded_generation(retriever)
    experiment_7_failure_case(retriever)
    experiment_8_retrieval_quality_effect(retriever)
